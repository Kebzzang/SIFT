...
void findPairs(vector<KeyPoint> & keypoints1, Mat & descriptors1,
	...
	for (int i = 0; i < descriptors1.rows; i++) {
		...
		Mat desc2 = descriptors2.row(i);
		int nn = nearestNeighbor(desc1, keypoints2, descriptors2);
		Mat NN = descriptors2.row(nn);
		// Refine matching points using ratio_based thresholding
		
		if (ratio_threshold) {
			double min_dist = euclidDistance(desc1,NN);
			double sec_min_distance = 1e6;
			for (int j = 0; j < descriptors2.rows; j++) {
				Mat des = descriptors2.row(j);
				double dist = euclidDistance(desc1, des); 
				if (dist > min_dist &&dist < sec_min_distance) {
					sec_min_distance = dist;
				}
			}
			if (min_dist / sec_min_distance >= RATIO_THR) continue;
		}

		// Refine matching points using cross-checking
		if (crossCheck) {
			int nn2 = nearestNeighbor(NN, keypoints1, descriptors1);
			if (i != nn2) continue;
		}
